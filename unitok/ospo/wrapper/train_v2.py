...
class UniTokWrapper(pl.LightningModule):
    def __init__(self, config, model, text_tokenizer, img_tokenizer):
        ...
        self.mask_gamma = simpo_config.get('mask_gamma', 1.0) # sharpening/softening of edges (gamma<1 → softer, >1 → sharper)
    ...
    def freeze_param(self):
        ...
        freeze = ['codebook', 'lm_head', 'multi_embedder', 'embed_tokens', 'ar_head'] # LLM ONLY TRAINING
        ...
        with torch.no_grad():
            for name, p in self.named_parameters():
                ...
                if torch.isnan(p.grad).any() or torch.isinf(p.grad).any():
                    print(f"[NaN/Inf GRAD AFTER BACKWARD] {name}")
                    break
    ...
    #             if p is None:
    #                 continue
    #         #    if p.grad is None:  # 필요하면 grad도 같이 확인
    #         #        continue
    #             if torch.isnan(p).any() or torch.isinf(p).any():
    #                 print(f"[NaN/Inf PARAM AFTER OPTIMIZER] {name}")
    #                 break
    ...
    def get_batch_loss_metrics(
        ...
        if self.sft_weight > 0.0:
            ...
            # print("# simpo loss: ", loss_outputs['simpo_loss'])
            # print("# sft loss: ", sft_loss)
            loss = (self.simpo_weight * loss_outputs['simpo_loss'] + self.sft_weight * sft_loss).mean()
            if do_logging:
            ...
            loss = (self.simpo_weight * loss_outputs['simpo_loss']).mean()
    ...
    # 매 코드북마다 (8) 특정 Spatial Location 에 위치한 토큰에 대해서 웨이트를 주는 방식
    def get_batch_logps(
        self,
        ...
        weight = loss_mask.to(dtype=per_token_logps.dtype) 
        ...
        if token_masks is not None:
            ...
            tm = token_masks
            # 2D grid, Flatten it
            if tm.dim() == 3 and tm.shape[-2:] == (16, 16): # Unitok Update: 사이즈 변경
                tm = tm.reshape(B, -1) # (B, Timg)  # torch.Size([16, 256])
            else:
                raise ValueError(
                    f"token_masks must be (B,16,16), but you gave {tm.shape}\n{tm[0]}"
                )


            """ SOFT MASKING VER. """
            # B, LK = weight.shape 
            # Unitok 에서 LK = K * L (=8*256)
            T_img = tm.shape[1] # 256

            # --- 1) Soften the binary token mask in 2D, then flatten back ---
            GRID_H = GRID_W = int((tm.shape[1]) ** 0.5)  # e.g., 256 -> 16
            assert GRID_H == 16 and GRID_W == 16, f"Unexpected grid size: {GRID_H}, {GRID_W}"
            ...
            mask_full = torch.ones((B, LK), device=weight.device, dtype=weight.dtype)
            # for b in range(B):
            ...
            for b in range(B):
                ...
                # T_img = number of spatial tokens (e.g. 256)
                assert span_len % T_img == 0, f"Image span length {span_len} not divisible by T_img {T_img}"
                K = span_len // T_img  # number of codebooks (e.g. 8)
                assert K == 8, f"Unexpected codebook size: {K}"

                # base boost per image token
                boost_img = 1.0 + self.mask_alpha * tm_soft[b]      # [T_img]

                # repeat the same mask for each codebook
                # ordering: [ℓ0_k0, ℓ0_k1, ..., ℓ0_k{K-1}, ℓ1_k0, ...]  or vice versa?
                # If your flattening is (ℓ, k) → index ℓ*K + k, this is correct:
                boost_rep = boost_img.repeat_interleave(K)          # [T_img * K]

                # write into full sequence span
                mask_full[b, s:e] = boost_rep.to(mask_full.dtype)
    ...
    def preprocess_train_batch(self, batch):
        ...
        if self.config.use_mask:
            item_ids, text_tokens, chosen_image_tensors, rejected_image_tensors, chosen_masks, rejected_masks = batch 
        else:
        ...
        if self.config.use_mask:
            ...
            preprocessed["chosen_token_mask"] = batched_chosen_masks.to(self.device)
            preprocessed["rejected_token_mask"] = batched_rejected_masks.to(self.device)
