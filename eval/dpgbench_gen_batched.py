...
class JanusTestWrapper(LightningModule):
    def __init__(self, model, text_tokenizer, processor, config):
        super().__init__()
        self.config = config
        self.model = model
        ...
        self.n_samples = PARALLEL_SIZE # 1
    ...
    def test_step(self, batch, batch_idx):
        ...
        for sample,data_idx in batch: 
            ...
            outpath = os.path.join(self.config.base.save_path, f"{data_idx:0>5}")
            os.makedirs(outpath, exist_ok=True)

            grid_path = os.path.join(self.config.base.save_path, "grid")
            os.makedirs(grid_path, exist_ok=True)

            sample_path = os.path.join(outpath, "samples")
            os.makedirs(sample_path, exist_ok=True)

            fpath = os.path.join(sample_path, "00003.png")
            ...
            grid_path_list.append(grid_path)
        ...
        for i in range(batch_size):
            # Grid 
            top_row = np.hstack((img_array[0][i], img_array[1][i]))
            bottom_row = np.hstack((img_array[2][i], img_array[3][i]))
            grid_image = np.vstack((top_row, bottom_row))
            PIL.Image.fromarray(grid_image).save(os.path.join(grid_path_list[i],f"{file_name_list[i]}.png"))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cfg_path", type=str, default="configs/eval/dpgbench.yaml")
