# v6: add loss weight scheduling / MSE Loss
...
class JanusProTrainWrapper(pl.LightningModule):
    ...
    # 추가
    def compute_pixel_loss(self, logits_img, target_img, tau, loss_type): # MSE(L2) or L1 loss
        """
        logits_img: (B, T_img, K)  visual-token logits (K = codebook_size)
        target_img: (B, 3, H, W)   GT image in [0,1] or [-1,1]
        tau: temperature for softmax (you can anneal from 1.0 → 0.3)
        loss_type: "mse" or "l1"
        """
