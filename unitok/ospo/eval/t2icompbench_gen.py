...
from unitok.ospo.dataclass import T2ICompBench
...
class UniTokTestWrapper(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        for sample in batch:
            # sample = batch[0] 
            prompt = self.get_prompt(self.text_tokenizer, sample['caption'])
        ...
        try:
            for seed in self.seed_list: # 보통 [42] 고정
                self.generate_batch(
...
def get_dataloader(config):
    train_dataset = T2ICompBench(config.task.data_path, category_list=config.task.category, split=config.task.split) 
...
def load_config():
    ...
    config = compose(config_name="t2icompbench", overrides=overrides)
...
def main(config: DictConfig):
    ...
    if config.model.type == "pl":
        wrapper = UniTokTestWrapper.load_from_checkpoint(checkpoint_path=config.model.ckpt_path, 
                                                            config=config, 
                                                            model=model,
                                                            text_tokenizer=text_tokenizer,
                                                            img_tokenizer=img_tokenizer,
                                                            strict=False) # 모델 전체를 학습시킨 것이 아니기 때문에 strict=False 필수
