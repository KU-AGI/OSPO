...
# v10 (v6): soft object mask on both SimPO & SFT
...
class JanusProTrainWrapper(pl.LightningModule):
    ...
    def concatenated_forward(
            ...
            all_logps, soft_weight = self.get_batch_logps( # [n_chosen + n_rejected, :]
                all_logits,
            ...
            if self.copo_mode:
                ...
                swap_chosen_logps, soft_weight = self.get_batch_logps(
                    swap_chosen_logits,
                    batch["swap_chosen_labels"],
                    token_masks=batch["chosen_token_mask"] if self.config.use_mask else None,
                    average_log_prob=True
                )
                return (chosen_logps, rejected_logps, swap_chosen_logps, chosen_logits, rejected_logits, swap_chosen_logits, chosen_labels, rejected_labels, soft_weight)

            else:
                return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, chosen_labels, rejected_labels, soft_weight)
    ...
    def get_batch_logps(
        ...
        if average_log_prob:
            ...
            return (weighted_sum / denom), weight
        else:
    ...
    def compute_sft_loss(self, policy_chosen_logits, policy_chosen_labels, soft_mask_weight):
        # logits: (B, T, V)
        # labels: (B, T)
        # soft_mask_weight: (B, T)

        logits = policy_chosen_logits[..., :-1, :]
        labels = policy_chosen_labels[..., 1:].clone()

        B, T, V = logits.shape

        # We only use chosen part.
        sw = soft_mask_weight[:B]
        

        if sw.dim() != 2 or sw.size(0) != B:
            raise ValueError(f"soft_mask_weight shape {tuple(sw.shape)} incompatible with batch B={B}")

        if sw.size(1) == T:
            pass  # already aligned
        elif sw.size(1) == T + 1:
            sw = sw[:, 1:]            # drop position 0 to match labels 1..T
        elif sw.size(1) > T:
            sw = sw[:, -T:]           # right-trim if longer
        else:
            raise ValueError(f"soft_mask_weight length {sw.size(1)} < T={T}; cannot align")

        # flatten for cross-entropy
        logits_flat = logits.reshape(-1, V)
        labels_flat = labels.reshape(-1)

        # standard CE without reduction
        ...
        # apply weights
        weighted_ce = ce_loss * sw                    

        # final average per batch item
        sft_loss = (weighted_ce.sum(dim=1) / sw.sum(dim=1).clamp_min(1e-8)).mean()
    ...
    def get_batch_loss_metrics(
        ...
        if self.sft_weight > 0.0 and self.pixel_weight > 0.0:
            ...
            if self.config.use_mask:
                sft_loss = self.compute_sft_loss(policy_chosen_logits, chosen_labels, soft_weight)
            else:
